...
def compute_prob_distributions_for_epig(model, scored_molecules, target_subset_size=1000):
...
def acquire_molecules(model, task, sampling, version):
    ...
    if "skill" not in sampling:
        ...
        scored_molecules = pd.read_csv(f"/home/vpalmacci/Projects/FTF4/FtF4/RUN_REINVENT/v{version}/{sampling}/{task}/{task}_1.csv")
        ...
        scored_molecules = pd.read_csv(f"/home/vpalmacci/Projects/FTF4/FtF4/RUN_REINVENT/v{version}/{sampling}/{task}.csv")
...
@click.option("-s", "--sampling", required=True, help="Specify the sampling strategy {random, greedy, molskill, greedyskill}")
...
def Main(sampling, dataset, iteration, version):
    ...
    for iter in range(iteration):
        ...
        scored_molecules.to_csv(f"/home/vpalmacci/Projects/FTF4/FtF4/RUN_REINVENT/v{version}/{sampling}/{task}/selected_{iter+1}.csv", index=False)

        processed_smiles = scored_molecules["SMILES"].values
        generated_fps = [compute_morgan(smi, 3) for smi in tqdm(processed_smiles)]
        
        generated_labels = (scored_molecules["interference"] >= 0.5).astype(int).values.tolist()

        ### Step 3: Append the generated data to the training set
        training_fps += generated_fps
        training_labels += generated_labels
        print("Percentage of positive samples in the training set: ", sum(training_labels) / len(training_labels)) # Print the ratio of positive samples in the training set
        ...
        with open(f"/home/vpalmacci/Projects/FTF4/FtF4/RUN_REINVENT/v{version}/{sampling}/{task}/{task}_{iter+1}.pkl", "wb") as f:
            pickle.dump(model, f)
