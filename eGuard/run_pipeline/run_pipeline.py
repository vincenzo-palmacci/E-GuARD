...
def reinvent_generation(task, acquisition, version):
    """
    Run REINVENT4 to generate molecules conditioned on the task
    """
    command = f"reinvent {version}/{acquisition}/{task}/config.toml"
...
def molskill_scoring(task, acquisition, version):
    """
    Run the molskill scoring script and return the output
    """

    molskill_scorer = (
        "../run_molskill/score_smiles.py"  # Path to the molskill scoring script
    )

    command = (
        f"conda run -n molskill python {molskill_scorer} {task} {acquisition} {version}"
    )
    result = subprocess.run(
        command, shell=True, check=True, capture_output=True, text=True
    )
...
@retry
def acquire_molecules(model, task, acquisition, version):
    """
    Generate and Acquire molecules to be added to the training set.
    """
    print("Generating molecules...")
    reinvent_generation(task, acquisition, version)
    ...
    if "skill" not in acquisition:
        ...
        try:
            molskill_scoring(task, acquisition, version)
        except:
...
def Main(source, acquisition, dataset, iteration, version):
    ...
    if not source:
        source = "alves"
    training_data = pd.read_csv(f"../../data/{source}/train/{task}.csv")
    ...
    for iter in range(iteration):
        ...
        toml_file = f"{version}/{acquisition}/{task}/config.toml"
        ...
        if iter == 0:
            model_path = f"../teacher/trained_models/{task}.pkl"
        else:
            model_path = f"/home/vpalmacci/Projects/E-GuARD/eGuard/run_pipeline/{version}/{acquisition}/{task}/{task}_{iter}.pkl"
        ...
        with open(
            f"/home/vpalmacci/Projects/E-GuARD/eGuard/run_pipeline/{version}/{acquisition}/{task}/{task}_{iter+1}.pkl",
            "wb",
        ) as f:
            pickle.dump(model, f)
